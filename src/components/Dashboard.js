    ...
    const loadAndTrainModel = async () => {
      try {
        ...
        try {
          ...
          const rows = csvText.split('\n').filter((row, index) => index > 1 && row.trim() !== '' && row.split(',').length === 10); // Remove the first two lines (URL and header row), filter out any empty rows, and ensure each row has the expected number of columns
